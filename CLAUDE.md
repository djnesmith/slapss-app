...
macOS pins a TCC grant to a **code requirement**, and the two databases pin
differently. This is why "Pause playing media when I join" stopped working on
2026-09-11 with nothing wrong in the code (reported as "slapss did not stop my
media this morning").

- **System db** (`/Library/Application Support/com.apple.TCC/TCC.db`) holds
  `kTCCServicePostEvent` and `kTCCServiceAccessibility` — both needed by
  `MediaPauser` — pinned by **certificate**:
  ...
  An **ad-hoc** build cannot satisfy that, ever — it has no certificate.
  `codesign -v -R=<that requirement> /Applications/slapss.app` returns *code
  failed to satisfy specified code requirement(s)*. System Settings still shows
  the toggle **on**, because the row is still there; macOS just drops the event.
- **User db** (`~/Library/Application Support/com.apple.TCC/TCC.db`) holds
  `kTCCServiceCalendar`, `kTCCServiceReminders` and `kTCCServiceAppleEvents`
  pinned by **`cdhash`** — one exact binary. Every rebuild changes the cdhash, so
  these always re-prompt on install, whatever identity is used.
